...
<template>
  <NuxtLayout name="charity">
    <h1 class="text-2xl font-medium my-10">
      {{ t('charity.allCountries') }}
    </h1>
    <ContentRenderer :value="data">
      <div class="grid grid-cols-1 md:grid-cols-2 gap-y">
        <div v-for="item in data.country" :key="item.slug" :class="item.level >= 3 ? 'col-span-full bg-red-100 shadow-lg hover:bg-red-200 shadow-red-200' : 'col-span-2 bg-gray-200 hover:bg-gray-400'" class="rounded mb-20">
          <NuxtLink
            :key="item" :to="`/charity/${item.slug}`" class="py-4 px-4 flex items-center relative justify-between"
          >
            <span
              v-if="item.level >= 3"
        ...
        <button
          ...
          :key="item.slug" class="rounded  col-span-2 bg-gray-200 hover:bg-gray-400"
          @click="toggle('open')"
        >
